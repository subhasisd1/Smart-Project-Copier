...
from project_mapper import PROJECT_EXCLUDES
...
class FileCopierApp(TkinterDnD.Tk):  # ✅ Only use TkinterDnD.Tk
    def __init__(self):
        ...
        self.project_types = {
            "React": tk.BooleanVar(),
            "Angular": tk.BooleanVar(),
            "Vue.js": tk.BooleanVar(),
            "Laravel": tk.BooleanVar(),
            "PHP": tk.BooleanVar(),
    ...
    def handle_drop(self, event):
        ...
        # 🔍 Detect and auto-select project types
        detected = self.detect_project_types(self.source_dirs)
        for proj_type, var in self.project_types.items():
            var.set(proj_type in detected)
    ...
    def create_widgets(self):
        ...
        self.chechbox_project_type_dropdown()
    ...
    def insert_node(self, parent, path, is_root=False):
        # Exclude based on project type
        if any(ex in path for ex in self.get_dynamic_excludes()):
            return
        
        # Exclude based on static/global patterns
    ...
    def get_dynamic_excludes(self):
        exclude_dirs = set()
        for project, var in self.project_types.items():
            if var.get():
                exclude_dirs.update(PROJECT_EXCLUDES.get(project, []))
        return exclude_dirs
    ...
    def chechbox_project_type_dropdown(self):
        self.project_types = {}
        project_options = list(PROJECT_EXCLUDES.keys())

        # Frame for dropdown
        project_frame = ttk.Frame(self)
        project_frame.pack(fill="x", padx=15, pady=(0, 10))
        ttk.Label(project_frame, text="Project Type:").pack(side="left")

        # Dropdown with checkboxes (using Menu)
        self.dropdown_btn = ttk.Menubutton(project_frame, text="Select Project Types", direction="below")
        self.menu = tk.Menu(self.dropdown_btn, tearoff=0)

        for project in project_options:
            var = tk.BooleanVar()
            self.project_types[project] = var
            self.menu.add_checkbutton(label=project, variable=var)

        self.dropdown_btn["menu"] = self.menu
        self.dropdown_btn.pack(side="left", padx=5)

    def detect_project_types(self, folder_paths):
        detected = set()
        for folder in folder_paths:
            try:
                items = set(os.listdir(folder))
                if "angular.json" in items:
                    detected.add("Angular")
                if "package.json" in items:
                    if "angular.json" not in items:
                        detected.add("React")  # fallback if angular not present
                    if "app.js" in items or "server.js" in items:
                        detected.add("Node.js")
                if "composer.json" in items:
                    detected.add("Laravel")
                    detected.add("PHP")
                if "requirements.txt" in items or "venv" in items or "__pycache__" in items:
                    detected.add("Python")
